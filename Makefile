...
install: xm2gb
	...
	$(INSTALL) -m 0755 xm2gb $(bindir)/xm2gb
	# 0644 is conventional for man pages
	$(INSTALL) -m 0644 xm2gb.1 $(MAN1DIR)/xm2gb.1
	# Compress if gzip is available (both Linux/macOS man read .gz)
	- gzip -f $(MAN1DIR)/xm2gb.1 >/dev/null 2>&1 || true
...
uninstall:
	-rm -f $(bindir)/xm2gb
	-rm -f $(MAN1DIR)/xm2gb.1 $(MAN1DIR)/xm2gb.1.gz
